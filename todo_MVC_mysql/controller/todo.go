...
func prepareData(w http.ResponseWriter) ViewData {
	// add db
...
func GetTodos(w http.ResponseWriter, r *http.Request) {
	// todos := todos
	data := prepareData(w) //prepareData(todos)
...
func UpdateTodo(w http.ResponseWriter, r *http.Request) {
	...
	// モデルのTodoを更新する
	err := model.UpdateTodoDB(id, item, completed)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		// エラーの場合の処理を記述する（例: エラーメッセージを表示する）
	}
